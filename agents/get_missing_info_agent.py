...
def get_missing_info(input_text: str) -> str:
    """
    يحلل البلاغ ويحدد فقط المعلومات الناقصة، ويعرضها مع جملة تمهيدية مناسبة.
    """
    ...
    مهمتك كمساعد طوارئ ذكي:
    - استخرج فقط الأسئلة الناقصة التي لم يذكرها المستخدم نهائيًا في البلاغ أو في أي رسالة سابقة داخل المحادثة.
    - نوع الطارئ ({emergency_type}) والنوع الفرعي ({emergency_subtype}) موجودان ومؤكدان، لا تسأل عنهما أبدًا مهما حصل.
    - تجاهل أي أسئلة عن الشرطة أو الاتصال بالجهات المختصة.
    - أعد النتيجة بصيغة JSON فقط بدون أي نص إضافي، مثال:
    [
      "السؤال الناقص الأول",
